 ...
 */
#define MASA_HEADER "\
Linked with MASA - Malleable Architecture for Sequence Aligners\n\
...
static void show_usage(char* program_name, Job* job) {
	...
	AbstractAlignerParameters* params = job->aligner->getParameters();
    printf ( "\033[1m%s:\033[0m\n", params->getUsageHeader());
    printf ( "%s\n\n", params->getUsage() );
}
...
static int fork_multi_process ( int count, Job* _job, const int* weights, int split_step) {
	...
	AbstractAlignerParameters* param = _job->aligner->getParameters();
...
void executeTracebackPipelined(Job* _job, Timer* timer, int ev_stage2, int ev_stage3, int ev_stage4, int ev_stage5, int ev_stage6) {
	int ev_stg2_wait = timer->createEvent("STG2_WAIT");
	int done = 0;
	if (stage2_pool_wait ( _job,  0, true) == -1) {
		done = 1;
	}
	timer->eventRecord(ev_stg2_wait);
	if (!done) {
		crosspoint_t crosspoint_r = stage2 ( _job, 0 );
		stage2_pool_send ( _job, crosspoint_r, true);
	}
	timer->eventRecord(ev_stage2);
	if (!done) {
		stage3 ( _job, 0 );
	...
	timer->eventRecord(ev_stage3);
	if (!done) {
		int mps = _job->stage4_maximum_partition_size;
		if (_job->stage4_maximum_partition_size < 1024) {
			_job->stage4_maximum_partition_size = 1024;
		}
		stage4 ( _job, 0 );
		if (stage4_pool_wait(_job, 0) < 0) done = 1;
		if (!done) {
			_job->stage4_maximum_partition_size = mps;
			stage4 ( _job, 1 );
		}
	}
	timer->eventRecord(ev_stage4);
	if (!done) {
		if (stage5 ( _job, 1 ) < 0) done = 1;
	}
	timer->eventRecord(ev_stage5);
	if (!done) {
		stage6 ( _job, 1 );
...
void executeTracebackPredicted(Job* _job, Timer* timer, int ev_stage2, int ev_stage3, int ev_stage4, int ev_stage5, int ev_stage6) {
	int ev_stg2_wait = timer->createEvent("STG2_WAIT");
	bool final = _job->getAlignerPool()->isLastNode();
	int id = 0;

	// Criar um método para isso (também duplicado no stage2)
	CrosspointsFile* crosspointsFile = new CrosspointsFile(
			_job->getCrosspointFile(STAGE_1, id));
	crosspointsFile->loadCrosspoints();
	crosspoint_t crosspoint_predicted = crosspointsFile->front();
	delete crosspointsFile;

	// Predicted
	crosspoint_t next = stage2 ( _job, id );
	stage2_pool_send ( _job, next, final);
	timer->eventRecord(ev_stage2);

	// this loop continuously predict until a final crosspoint has been received
	while (!final) {
		id++;
		bool must_be_final = (id >= 5);
		int next_id = stage2_pool_wait ( _job, id, must_be_final, &final, &next);
		timer->eventRecord(ev_stg2_wait);
		if (next_id == id) {
			next = stage2 ( _job, id );
			timer->eventRecord(ev_stage2);
	...
	if (!done) {
		...
		stage4 ( _job, id );
		if (stage4_pool_wait(_job, id) < 0) done = 1;
		if (!done) {
			_job->stage4_maximum_partition_size = mps;
			stage4 ( _job, id+1 );
		}
...
int libmasa_entry_point(int argc, char** argv, IAligner* aligner) {
    printf ( MASA_HEADER );
	//printf ("Linked with libmasa - Malleable Architecture for Sequence Aligners.\n");
		...
		while ( 1 ) {
			...
			case ARG_BLOCKS:
				if ( optarg != NULL )  {
					...
					// TODO XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXxx COLOCAR NO PARAMETER DO CUDA xxxxxxxx
					if ( _job->blocks > BLOCKS_COUNT ) {
						stringstream out;
						out << "Blocks count cannot be greater than " << BLOCKS_COUNT << ".";
			...
			case '?': {
				optind--;
				int c2 = getopt_long ( argc, argv, ":", _job->aligner->getParameters()->getArguments(), NULL );
				if (c2 == '?') {
					throw IllegalArgumentException("Invalid Option.", current_arg);
				} else if (c2 == ':') {
					throw IllegalArgumentException("An argument must be supplied to this parameter.", current_arg);
				} else {
					int ret = _job->aligner->getParameters()->processArgument(c2, current_arg, optarg);
					if (ret) {
    ...
    if (_job->peer_listen_port >=0 ) {
    	MasaNet* peer = new MasaNet(TYPE_PROCESSING_NODE, "CUDAlign");
    ...
    if ( phase == ALL_STAGES ) {
    	...
    	if (_job->getAlignerPool() == NULL) {
    		...
    		if (_job->predicted_traceback) {
    			executeTracebackPredicted(_job, &timer, ev_stage2, ev_stage3, ev_stage4, ev_stage5, ev_stage6);
    		} else {
        		executeTracebackPipelined(_job, &timer, ev_stage2, ev_stage3, ev_stage4, ev_stage5, ev_stage6);
    		}
    	}
