...
/**
 * Recomended block size for better performance
 */
 ...
 */
AbstractBlockAligner::AbstractBlockAligner(AbstractBlockProcessor* blockProcessor) {
	/*
	 * Initializations
	 */
	col = NULL;
 ...
 */
AbstractAlignerParameters* AbstractBlockAligner::getParameters() {
	return this->params;
}
...
Grid* AbstractBlockAligner::configureGrid(Partition partition) {
	...
	if (params->getBlockHeight() > 0) {
		...
		if (!mustDispatchLastColumn()) {
			int height = RECOMMENDED_BLOCK_SIZE;
			if (grid->getHeight() / height < maxThreads * 2) {
				height = grid->getHeight() / (maxThreads * 2);
			}
			grid->setBlockHeight(height);
		} else {
			grid->setBlockHeight(grid->getWidth()/(maxThreads*2)/2);
		}
	}
	...
	if (params->getBlockWidth() > 0) {
		...
		// Automatic configuration
		if (!mustDispatchLastColumn()) {
			int width = RECOMMENDED_BLOCK_SIZE;
			if (grid->getWidth() / width < maxThreads * 2) {
				width = grid->getWidth() / (maxThreads * 2);
			}
			grid->setBlockWidth(width);
		} else {
			grid->splitGridHorizontally(maxThreads * 2);
		}
	}
...
void AbstractBlockAligner::alignPartition(Partition partition) {
	...
	receiveFirstColumn(&firstColumnTail, 1);
	receiveFirstRow(&firstRowTail, 1);
