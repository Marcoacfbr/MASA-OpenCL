...
void AbstractDiagonalAligner::processNextIteration() {
	...
	statTotalCells += (jb1-jb0)* gridHeight;
...
void AbstractDiagonalAligner::printStatistics(FILE* file) {
	...
	fprintf(file, "\n=====  PRUNING STATS   =====\n");
	fprintf(file, "Pruned Blocks: %d %d\n", statPrunedBlocksLeft, statPrunedBlocksRight);
	fprintf(file, "Pruned Blocks: %.4f %.4f\n",
 ...
 * CUDAligner::clearStatistics method.
 *
...
void AbstractDiagonalAligner::loadFirstRow() {
	...
	firstRowTail.h = chunk[j1 - 1].h;
	firstRowTail.f = chunk[j1 - 1].f;
...
void AbstractDiagonalAligner::loadFirstColumn() {
    ...
    if (pos_i < partition.getHeight()) {
        ...
        h_loadColumn[0] = firstColumnTail;
		...
		firstColumnTail = h_loadColumn[len];
