...
//	sraPartition->setFirstRowReader(firstRow);
//	sraPartition->setFirstColumnReader(firstColumn);
//	sraPartition->setLastColumnWriter(lastColumn);
//	sraPartition->setLastRowWriter(lastRow);

//	if (job->getSRALimit() > 0) {
		sra = job->getSpecialRowsArea(STAGE_1, 0);


//		SpecialRowsPartition* p1 = sra->createPartition(i0, j0, i1, j1);
//		p1->setFirstRowReader(firstRow);
//		p1->setFirstColumnReader(firstColumn);
//		p1->setLastRowWriter(lastRow);
//		p1->setLastColumnWriter(lastColumn);
...
//		SpecialRowsPartition* p1 = sra->createPartition(i0, j0, i1, (j0+j1)/2);
//		SpecialRowsPartition* p2 = sra->createPartition(i0, (j0+j1)/2, i1, j1);
//		p1->setFirstRowReader(firstRow);
//		p2->setFirstRowReader(new SplitCellsReader(firstRow, (j0+j1)/2 - j0, true)); // integer division
//
//		p1->setFirstColumnReader(firstColumn);
//
//		p2->setLastColumnWriter(lastColumn);
//
//		p1->setLastRowWriter(lastRow);
//		p2->setLastRowWriter(lastRow);
//
//		p1->createChain(p2);

//
//
//		SpecialRowsPartition* p1 = sra->createPartition(i0, j0, (i0+i1)/2, (j0+j1)/2);
//		SpecialRowsPartition* p2 = sra->createPartition(i0, (j0+j1)/2, (i0+i1)/2, j1);
//		SpecialRowsPartition* p3 = sra->createPartition((i0+i1)/2, j0, i1, (j0+j1)/2);
//		SpecialRowsPartition* p4 = sra->createPartition((i0+i1)/2, (j0+j1)/2, i1, j1);
//
//
//
//		p1->setFirstRowReader(firstRow);
...
//
//		p1->setFirstColumnReader(firstColumn);
//		p3->setFirstColumnReader(new SplitCellsReader(firstColumn, (i0+i1)/2 - i0, true)); // integer division
//
//		p2->setLastColumnWriter(lastColumn);
//		p4->setLastColumnWriter(lastColumn);
//
//		p3->setLastRowWriter(lastRow);
//		p4->setLastRowWriter(lastRow);
//
//		p1->createChain(p2);
//		p1->createChain(p3);
//		p2->createChain(p4);
...
//	}
	...
	// TODO DELETE
//	if (readColumn != NULL) {
//		fprintf(stats, "Blocked Input Time: %.3f\n",
//				readColumn->getBlockingReadTime());
//		delete readColumn;
//		readColumn = NULL;
//	}
//	if (writeColumn != NULL) {
//		fprintf(stats, "Output Buffer Usage at End: %d\n",
//				writeColumn->getUsage());
//		writeColumn->waitEmptyBuffer();
//		fprintf(stats, "Blocked Output Time: %.3f\n",
//				writeColumn->getBlockingWriteTime());
//		delete writeColumn;
//		writeColumn = NULL;
//	}
