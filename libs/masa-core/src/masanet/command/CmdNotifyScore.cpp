...
Command* CmdNotifyScore::creator() {
	return new CmdNotifyScore();
}

int CmdNotifyScore::getId() {
	return COMMAND_NOTIFY_SCORE;
}

void CmdNotifyScore::send(Peer* socket) {
	socket->send_int32(i);
	socket->send_int32(j);
	socket->send_int32(score);
}

void CmdNotifyScore::receive(Peer* socket) {
	i = socket->recv_int32();
	j = socket->recv_int32();
	score = socket->recv_int32();
}
