...
/**
 * @file OpenCLAligner.cu
 * @brief OpenCLAligner related kernels and wrappers.
 *
 * This file contains all the kernels and some wrappers for the
 * OpenCLAligner class. Ensure that all OpenCLAligner.* files are compiled
...
__device__ void kernel_sw4(const int i, const int j, const unsigned char s1, const uchar4 ss,
	...
	kernel_sw(s1, ss.x, &left_e->x, up_f, left_h.x, diag_h  , up_h  , &curr_h->x);
	kernel_sw(s1, ss.y, &left_e->y, up_f, left_h.y, left_h.x, curr_h->x, &curr_h->y);
	kernel_sw(s1, ss.z, &left_e->z, up_f, left_h.z, left_h.y, curr_h->y, &curr_h->z);
	kernel_sw(s1, ss.w, &left_e->w, up_f, left_h.w, left_h.z, curr_h->z, &curr_h->w);
}
		...
		int4* flushColumn_h, int4* flushColumn_e)
{
	...
	for (; _k; _k--) {
		...
		/* Loop-unrolling #1 */
		kernel_check_bound(i0, d_split[0], i1, d_split[gridDim.x], &i, &j, &ss, &left_e, &diag_h, &left_h, &flush_id, loadColumn_h, loadColumn_e, flushColumn_h, flushColumn_e, idx, blockDim.x*gridDim.x*ALPHA);
		if (flush_id) {
			unsigned char s1;
			kernel_load(idx, 1, j, busH, &up_h, &up_f, &s1);
			kernel_sw4(i, j, s1, ss, &left_e, &up_f, left_h, diag_h , up_h , &curr_h, flush_id);
			kernel_check_max4(i, j, max, max_pos, &curr_h, true, flush_id);
			kernel_flush(i, j, idx, 0, busH, extraH, &curr_h, up_f, THREADS_COUNT-1, flush_id);
		}
		j++;
		__syncthreads(); // barrier

		/* Loop-unrolling #2 */
		kernel_check_bound(i0, d_split[0], i1, d_split[gridDim.x], &i, &j, &ss, &left_e, &up_h, &curr_h, &flush_id, loadColumn_h, loadColumn_e, flushColumn_h, flushColumn_e, idx, blockDim.x*gridDim.x*ALPHA);
		if (flush_id) {
			unsigned char s1;
			kernel_load(idx, 0, j, busH, &diag_h, &up_f, &s1);
			kernel_sw4(i, j, s1, ss, &left_e, &up_f, curr_h, up_h , diag_h, &left_h, flush_id);
			kernel_check_max4(i, j, max, max_pos, &left_h, true, flush_id);
			kernel_flush(i, j, idx, 1, busH, extraH, &left_h, up_f, THREADS_COUNT-1, flush_id);
		}
		j++;
		__syncthreads(); // barrier
	}

	{
		...
		int up_h;
		int up_f;
		kernel_check_bound(i0, j0, i1, j1, &i, &j, &ss, &left_e, &diag_h, &left_h, &flush_id, loadColumn_h, loadColumn_e, flushColumn_h, flushColumn_e, idx, blockDim.x*gridDim.x*ALPHA);
		if (flush_id) {
			unsigned char s1;
			kernel_load(idx, 1, j, busH, &up_h, &up_f, &s1);
			kernel_sw4(i, j, s1, ss, &left_e, &up_f, left_h, diag_h, up_h, &curr_h, flush_id);
		...
		kernel_check_bound(i0, j0, i1, j1, &i, &j, &ss, &left_e, &up_h, &curr_h, &flush_id, loadColumn_h, loadColumn_e, flushColumn_h, flushColumn_e, idx, blockDim.x*gridDim.x*ALPHA);
		busV_h[tidx] = curr_h;
		busV_e[tidx] = left_e;
		...
		const int flush_id)
{
	...
	if (_k&1) { // if odd
		...
		unsigned char s1;
		kernel_load(idx, 1, j, busH, &up_h, &up_f, &s1);
		kernel_sw4(i, j, s1, ss, &left_e, &up_f, left_h, diag_h, up_h, &cur_h, flush_id);
		kernel_check_max4(i, j, max, max_pos, &cur_h, false, flush_id);
		kernel_flush(i, j, idx, 0, busH, extraH, &cur_h, up_f, THREADS_COUNT-1, flush_id);
		j++;
		__syncthreads();
	...
	for (; _k; _k--) {
		...
		/* Loop-unrolling #2 */
		kernel_load(idx, 0, j, busH, &diag_h, &up_f, &s1);
		kernel_sw4(i, j, s1, ss, &left_e, &up_f, cur_h, up_h, diag_h, &left_h, flush_id);
		kernel_check_max4(i, j, max, max_pos, &left_h, false, flush_id);
		kernel_flush(i, j, idx, 1, busH, extraH, &left_h, up_f, THREADS_COUNT-1, flush_id);
		j++;
		__syncthreads();
		...
		int4* busV_h, int4* busV_e, int3* busV_o)
{
	...
	if (i < i1) {
		process_internal_diagonals_long_phase(xLen, idx, tidx, i, j, busH, extraH, busV_h, busV_e, busV_o, &max, &max_pos, -1);
    }
		...
		int4* flushColumn_h, int4* flushColumn_e)
{
    ...
    for (; _k; _k--) {
        ...
        int  up_h;
        int  up_f;
		kernel_check_bound(i0, j0, i1, j1, &i, &j, &ss, &left_e, &diag_h, &left_h, &flush_id, loadColumn_h, loadColumn_e, flushColumn_h, flushColumn_e, idx, blockDim.x*ALPHA);
        if (flush_id) {
            unsigned char s1;
            kernel_load(idx, index, j, busH, &up_h, &up_f, &s1);
			kernel_sw4(i, j, s1, ss, &left_e, &up_f, left_h, diag_h  , up_h  , &curr_h, flush_id);
			kernel_check_max4(i, j, max, max_pos, &curr_h, true, flush_id);
            kernel_flush(i, j, idx, 1-index, busH, extraH, &curr_h, up_f, blockDim.x-1, flush_id);
        }
        index = 1-index;
        j++;
        diag_h = up_h;
		...
		int4* flushColumn_h, int4* flushColumn_e)
{
	...
	if (i < i1) {
		process_internal_diagonals_single_phase(xLen, idx, i, j, i0, j0, i1, j1, busH, extraH, busV_h, busV_e, busV_o, &max, &max_pos, -1, loadColumn_h, loadColumn_e, flushColumn_h, flushColumn_e);
	}
...
//void OpenCLAligner::initializeBusHUngapped(const int p0, const int p1) {
//	dim3 threads(512, 1, 1);
