...
/**
 * Instantiates the OpenCLAligner object.
 */
OpenCLAligner::OpenCLAligner() {
 ...
 * 	this Aligner.
 */
aligner_capabilities_t OpenCLAligner::getCapabilities() {
	aligner_capabilities_t capabilities;
	capabilities.smith_waterman 			= SUPPORTED;
	capabilities.needleman_wunsch 			= NOT_SUPPORTED;
	capabilities.block_pruning 				= SUPPORTED;
	capabilities.customize_first_column 	= NOT_SUPPORTED;
	capabilities.customize_first_row 		= NOT_SUPPORTED;
	capabilities.dispatch_last_cell 		= NOT_SUPPORTED;
	capabilities.dispatch_last_column 		= NOT_SUPPORTED;
	capabilities.dispatch_last_row 			= NOT_SUPPORTED;
	capabilities.dispatch_special_column 	= NOT_SUPPORTED;
	capabilities.dispatch_special_row 		= NOT_SUPPORTED;
 ...
 * @return the OpenCLAlignerParameters associated with this aligner.
 */
AbstractAlignerParameters* OpenCLAligner::getParameters() {
	return this->params;
}
 ...
 */
const score_params_t* OpenCLAligner::getScoreParameters() {
	return &score_params;
}

/**
 * Initialize the aligner. This method is called only once during the
 * aligner lifetime. Here we already have the OpenCLAlignerParameters
...
void OpenCLAligner::initialize() {
	...
	if (this->params->getForkId() != NOT_FORKED_INSTANCE) {
		...
		int gpus = clgetAvailableGPU(ids, MAX_GPUS);
	...
	op.glcontext = CreateContext(glplatform);
	op.gldevice = clSetupDevice(glplatform);
	op.gldevice = clselectGPU(NULL);
	op.glcommand = clCreateCommandQueue(op.glcontext, op.gldevice, 0, &errNum);
	if (!op.glcommand)
	 	exit(0);
	...
	if (clgetCompiledCapability() > clgetDevCapability(op.gldevice)) {
			fprintf(stderr, "FATAL: Unsupported GPU Device Capability. (%d < %d).\n", clgetDevCapability(op.gldevice), clgetCompiledCapability());
			fprintf(stderr, "Maybe you should try to recompile the code with an old cuda architecture.\n");
			exit(1);
	}

	multiprocessors = clgetGPUMultiprocessors(op.gldevice);

	// Alocates the default structures.
	clallocateGlobalStructures();
 ...
 */
void OpenCLAligner::finalize() {
	//freeGlobalStructures();
	clfreeGlobalStructures();
	//cudaDeviceReset();
	Cleanup(op.glcontext, op.glprogram);
 ...
 */
void OpenCLAligner::initializeDiagonals() {
	//copy_split(getGrid()->getBlockSplitHorizontal(), getGrid()->getGridWidth());
	cl_copy_split(&op, getGrid()->getBlockSplitHorizontal(), getGrid()->getGridWidth());
}
 ...
 */
void OpenCLAligner::processDiagonal(int diagonal, int windowLeft, int windowRight) {
	int2 cutBlock;
	//cutBlock = make_int2(windowLeft, windowRight);
        cutBlock.x = windowLeft;
        cutBlock.y = windowRight;

	cl_lauch_external_diagonals(op.glcontext, op.gldevice, getFirstColumnInitType(), mustDispatchLastColumn(),
			getRecurrenceType(), mustDispatchScores(),
			getGrid()->getGridWidth(), getThreadCount(), getPartition().getI0(), getPartition().getI1(), diagonal, cutBlock, &cuda, &op);
}

/**
 * Finalizes structures after processing diagonals.
 */
void OpenCLAligner::finalizeDiagonals() {
	// Does nothing
}
 ...
 */
void OpenCLAligner::setSequences(
		const char* seq0, const char* seq1, int seq0_len, int seq1_len) {
	if (DEBUG) printf("OpenCLAligner::allocateStructures()\n");
	...
	clgetMemoryUsage(&usedMemoryBefore);
	...
	op.d_seq0 = clallocSeq(op.glcontext, op.gldevice, seq0, seq0_len, seq0_padding, '\0');
	//cuda.d_seq1 = allocCudaSeq(seq1, seq1_len, seq1_padding, '\0');
	op.d_seq1 = clallocSeq(op.glcontext, op.gldevice,seq1, seq1_len, seq1_padding, '\0');
	//cuda.busH_size = bus_size;


	op.busH_size = bus_size;
	cl_bind_textures(&op, seq0, seq0_len+seq0_padding, seq1, seq1_len+seq1_padding);

	//cuda.d_busH = (int2*)  allocCuda0(bus_size);
	op.d_busH = clalloc0(op.glcontext, op.gldevice, bus_size);

    size_t usedMemory;
	//getMemoryUsage(&usedMemory);
    clgetMemoryUsage(&usedMemory);
 ...
 */
void OpenCLAligner::unsetSequences() {
	if (DEBUG) printf("OpenCLAligner::freeStructures()\n");

	free(host.h_busH);

	//cutilSafeCall(cudaFree(cuda.d_seq0));
	clReleaseMemObject(op.d_seq0);
	//cutilSafeCall(cudaFree(cuda.d_seq1));
	clReleaseMemObject(op.d_seq1);
	cl_unbind_textures();

	//cutilSafeCall(cudaFree(cuda.d_busH));
	clReleaseMemObject(op.d_busH);
	op.d_busH = NULL;
 ...
 */
int OpenCLAligner::getBlockHeight() {
 ...
 * than OpenCLAlignerParameters::blocks.
 */
int OpenCLAligner::getGridWidth(const int width) {
 ...
 */
cell_t* OpenCLAligner::getLastRow(int j, int len) {
	...
	command = clCreateCommandQueue(op.glcontext, op.gldevice, 0, &errNum);
	if (!command)
		exit(0);
	if (j-shift < j0) {
		int diff = j0 - (j-shift);
		...
		if (DEBUG) printf("EXTRA x0: %d  diff: %d   xLen: %d!  mem: %p\n", j, diff, len, op.d_extraH);
	    //cutilSafeCall(cudaMemcpy(host.h_busH+j, cuda.d_extraH, (diff)*sizeof(int2), cudaMemcpyDeviceToHost));
		errNum =  clEnqueueReadBuffer(command, op.d_extraH, CL_TRUE,0,(diff)*sizeof(int2), host.h_busH+j, 0, NULL, NULL);
	    if (len-diff > 0) {
	    	//cutilSafeCall(cudaMemcpy(host.h_busH+j+diff, cuda.d_busH+j0,(len-diff)*sizeof(int2), cudaMemcpyDeviceToHost));
	    	errNum =  clEnqueueReadBuffer(command, op.d_busH, CL_TRUE,j0*sizeof(int2),(len-diff)*sizeof(int2), host.h_busH+j+diff, 0, NULL, NULL);
	    }
	} else {
	    //cutilSafeCall(cudaMemcpy(host.h_busH+j, cuda.d_busH+j-shift,(len)*sizeof(int2), cudaMemcpyDeviceToHost));
		errNum =  clEnqueueReadBuffer(command, op.d_busH, CL_TRUE,(j-shift)*sizeof(int2),(len)*sizeof(int2), host.h_busH+j, 0, NULL, NULL);
	}
	clFinish(command);
 ...
 * @param len number of cells to be loaded.
 */
cell_t* OpenCLAligner::getSpecialRow(int j, int len) {
	//cudaStreamSynchronize(0);
	cl_int errNum;
	cl_command_queue command = op.glcommand;

	//cutilSafeCall(cudaMemcpy(host.h_busH + j, cuda.d_busH + j, len * sizeof(int2),cudaMemcpyDeviceToHost));
	errNum =  clEnqueueReadBuffer(command, op.d_busH, CL_TRUE,(j)*sizeof(int2),(len)*sizeof(int2), host.h_busH+j, 0, NULL, NULL);
	//cudaStreamSynchronize(0);
	clFlush(command);
	clFinish(command);
 ...
 * @param len number of cells to be loaded.
 */
cell_t* OpenCLAligner::getLastColumn(int i, int len) {
	// unused parameters.
	...
	errNum =  clEnqueueReadBuffer(command, op.d_flushColumnH, CL_TRUE,0,_len*sizeof(int4), host.h_flushColumnH, 0, NULL, NULL);
	//cutilSafeCall(cudaMemcpy(host.h_flushColumnE, cuda.d_flushColumnE, _len*sizeof(int4), cudaMemcpyDeviceToHost));
	errNum =  clEnqueueReadBuffer(command, op.d_flushColumnE, CL_TRUE,0,_len*sizeof(int4), host.h_flushColumnE, 0, NULL, NULL);
 ...
 */
score_t* OpenCLAligner::getBlockScores() {
	...
	errNum =  clEnqueueReadBuffer(command, op.d_blockResult, CL_TRUE,0,sizeof(int4)*blocks, host.h_blockResult, 0, NULL, NULL);
 ...
 */
void OpenCLAligner::setFirstRow(const cell_t* cells, int j, int len) {
	cl_int errNum;
	cl_command_queue command = op.glcommand;
	if (DEBUG) fprintf(stderr, "OpenCLAligner::setFirstRow(..., %d, %d)\n", j, len);
	//cutilSafeCall(cudaMemcpy(cuda.d_busH + j, cells, len*sizeof(int2), cudaMemcpyHostToDevice));
	errNum =  clEnqueueWriteBuffer(command, op.d_busH, CL_TRUE,j*sizeof(int2),len*sizeof(int2), cells, 0, NULL, NULL);
	clFlush(command);
	clFinish(command);
 ...
 * @param len length of the vector.
 */
void OpenCLAligner::setFirstColumn(const cell_t* cells, int i, int len) {
	...
	//cutilSafeCall(cudaMemcpy(cuda.d_loadColumnH, host.h_loadColumnH, (getBlockHeight() + 4) * sizeof(int), cudaMemcpyHostToDevice));
	errNum =  clEnqueueWriteBuffer(command, op.d_loadColumnH, CL_TRUE,0,(getBlockHeight() + 4) * sizeof(int), host.h_loadColumnH, 0, NULL, NULL);
	//cutilSafeCall(cudaMemcpy(cuda.d_loadColumnE, host.h_loadColumnE, (getBlockHeight() + 4) * sizeof(int), cudaMemcpyHostToDevice));
	errNum =  clEnqueueWriteBuffer(command, op.d_loadColumnE, CL_TRUE,0,(getBlockHeight() + 4) * sizeof(int), host.h_loadColumnE, 0, NULL, NULL);
	clFlush(command);
	clFinish(command);
 ...
 */
void OpenCLAligner::clearStatistics() {
 ...
 */
void OpenCLAligner::printInitialStatistics(FILE* file) {
	AbstractDiagonalAligner::printInitialStatistics(file);

	fprintf(file, "\n===== GPU DEVICE INFO  =====\n");

	clprintDevProp(file, op.gldevice);
	...
	fprintf(file, "  CUDA ARCH: %.1f\n", clgetCompiledCapability()/100.0f);
 ...
 * @param file handler to print out the statistics.
 */
void OpenCLAligner::printStageStatistics(FILE* file) {
 ...
 * @param file handler to print out the statistics.
 */
void OpenCLAligner::printFinalStatistics(FILE* file) {
 ...
 * @param file handler to print out the statistics.
 */
void OpenCLAligner::printStatistics(FILE* file) {
 ...
 */
void OpenCLAligner::allocateGlobalStructures() {
/*	//getMemoryUsage(&statInitialUsedMemory, &statTotalMem);
	clgetMemoryUsage(&statInitialUsedMemory, &statTotalMem);
...
void OpenCLAligner::clallocateGlobalStructures() {
	//getMemoryUsage(&statInitialUsedMemory, &statTotalMem);
	...
	op.d_extraH       = clalloc0(op.glcontext, op.gldevice, THREADS_COUNT*sizeof(int2));
	op.d_flushColumnH = clalloc0(op.glcontext, op.gldevice, THREADS_COUNT*sizeof(int4));
	op.d_flushColumnE = clalloc0(op.glcontext, op.gldevice, THREADS_COUNT*sizeof(int4));
	op.d_loadColumnH  = clalloc0(op.glcontext, op.gldevice, (THREADS_COUNT+1)*sizeof(int4));
	op.d_loadColumnE  = clalloc0(op.glcontext, op.gldevice, (THREADS_COUNT+1)*sizeof(int4));
	op.d_blockResult  = clalloc0(op.glcontext, op.gldevice, MAX_BLOCKS_COUNT*sizeof(int4));
	op.d_busV_h       = clalloc0(op.glcontext, op.gldevice, MAX_GRID_HEIGHT*sizeof(int4));
	op.d_busV_e       = clalloc0(op.glcontext, op.gldevice, MAX_GRID_HEIGHT*sizeof(int4));
	op.d_busV_o       = clalloc0(op.glcontext, op.gldevice, MAX_GRID_HEIGHT*sizeof(int3));

	op.d_split_m      = clalloc0(op.glcontext, op.gldevice, (MAX_BLOCKS_COUNT+1)*sizeof(int));

	//op.d_colx         = clalloc0(op.glcontext, op.gldevice, 2*(THREADS_COUNT)*sizeof(int));
	//op.d_coly         = clalloc0(op.glcontext, op.gldevice, 2*(THREADS_COUNT)*sizeof(int));
 ...
 */
void OpenCLAligner::freeGlobalStructures() {
...
void OpenCLAligner::clfreeGlobalStructures() {
	if (DEBUG) printf("freeFixedSizeStructures()\n");
	free(host.h_extraH);
	free(host.h_flushColumnH);
	free(host.h_flushColumnE);
	...
	clReleaseMemObject(op.d_extraH);
	clReleaseMemObject(op.d_flushColumnH);
	clReleaseMemObject(op.d_flushColumnE);
	clReleaseMemObject(op.d_loadColumnH);
	clReleaseMemObject(op.d_loadColumnE);
	clReleaseMemObject(op.d_blockResult);
	clReleaseMemObject(op.d_busV_h);
	clReleaseMemObject(op.d_busV_e);
	clReleaseMemObject(op.d_busV_o);

	clReleaseCommandQueue(op.glcommand);
}
